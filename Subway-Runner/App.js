...
export default function App() {
    ...
    const [playerPosition, setPlayerPosition] = useState({ x: width / 2 - 25, y: playerY });
    ...
    function isColliding(playerPosition, obstacle) {
        obstaclePosition = obstacle.position;
        if (!playerPosition || !obstacle) return false;
        // player left, right, top, bottom
        const pl = playerPosition.x;
        const pr = pl + playerWidth;
        const pt = playerPosition.y;
        const pb = pt + playerWidth;

        // obstacle left, right, top, bottom
        const ol = obstaclePosition.x;
        const or = ol + (obstacle.size?.height || 50);
        const ot = obstaclePosition.y;
        const ob = ot + (obstacle.size?.width || 50);
            ...
            gameLoopRef.current = setInterval(() => {
                ...
                setObstacles(prevObstacles => {
                    ...
                    for (let obstacle of updatedObstacles) {
                        if (isColliding(playerPosition, obstacle)) {
                            setGameOver(true);
                            if (gameLoopRef.current) {
                                clearInterval(gameLoopRef.current);
                            }
                            return updatedObstacles; // Early return, game over
                        }
                    }
    ...
    const resetGame = () => {
        ...
        setScore(0);
        setPlayerPosition({ x: width / 2 - 25, y: playerY });
        setObstacles([]);
